...
occurred on the socket within ~TIMEOUT~ seconds or an event occurred
on the socket but when testing the socket it determined that it does
not satisfy the criteria for any of the symbols in
~SOCKET-EVENTS~. The last scenario is unfortunate and unavoidable as
some operating systems provide no means of discriminating all of the
above events with polling alone. The only way to perform the
discrimination is to inspect the socket directly.
...
event on any socket of the sockets. The return value is a list
containing the results of performing ~POLL-SOCKET~ on that socket
alone. For example
#+begin_src lisp
