...
function returns an object that implements the future connection
protocol and allows an application to ascertain the status of the
connecting process. There are two functions in this protocol, the
first is ~DETERMINEDP~.
#+begin_src lisp
(defgeneric determinedp (future-connection))
#+end_src
A non ~NIL~ return value of ~DETERMINEDP~ indicates that establishing
the connection has finished. 

This function ~CONNECTION-STREAM~ will return an object that
implements the stream protocol or signal an error if the connection
...
#+end_src
This function will write the data as soon all other data has been
...
#+begin_src lisp
(defun make-ipv4-tcp-server (host-address port &key reuse-socket-address))
#+end_src
