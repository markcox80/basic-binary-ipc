...
(define-binary-type binary-utf8-string ()
  (:reader (in)
    (let* ((number-of-bytes (read-value 'binary-uint32 in))
	   (bytes (make-array number-of-bytes :element-type '(unsigned-byte 8)))
	   (bytes-read (read-sequence bytes in)))
      (assert (= number-of-bytes bytes-read))
...
(define-binary-type binary-integer ()
  (:reader (in)
    (let ((sign            (read-value 'binary-int8 in))
	  (number-of-words (read-value 'binary-uint32 in))
	  (rv              0))
      (declare (type (integer 0) rv))
  ...
  (:writer (out value)
    (let ((words nil)
	  (sign (signum value)))
      (loop
	 :with value := (abs value)
	 :until (zerop value)
	 :do
	 (push (ldb (byte 32 0) value) words)
	 (setf value (ash value -32)))
      (write-value 'binary-int8  out sign)
