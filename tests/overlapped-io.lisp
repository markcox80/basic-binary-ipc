...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defun do-with-ipv4-connection (address port local-address local-port function)
    (let ((local-address (or local-address +inaddr-any+))
	  (local-port (or local-port 0)))
      (with-socket (socket (make-socket :af-inet :sock-stream :ipproto-tcp))
	(with-request (request (make-instance 'connect-ipv4-request))
	  (connect-ipv4 socket address port request local-address local-port)
	  (with-socket (socket socket)
	    (funcall function socket request))))))
  
  (defmacro with-ipv4-connection ((socket request address port &optional local-address local-port) &body body)
    `(do-with-ipv4-connection ,address ,port			      
...
(define-test ipv4-connection/specific-ports
  (let ((server-port (random-port))
	(client-port (random-port))
	(address "127.0.0.1")
	(buffer-length (minimum-accept-ipv4-buffer-size)))
    (with-socket (server (make-ipv4-server address server-port))
      (with-socket (remote-client (make-socket :af-inet :sock-stream :ipproto-tcp))
	(cffi:with-foreign-objects ((buffer :uint8 buffer-length))
	  (with-request (accept (accept-ipv4 server remote-client buffer buffer-length))
	    (with-ipv4-connection (client connection-request address server-port address client-port)
	      (declare (ignore client))
	      (wait-for-request accept 10)
	      (assert-true (completedp accept))
	      (assert-true (succeededp accept))
	      ...
	      (assert-true (succeededp connection-request))
	      (assert-equal address (remote-address connection-request))
	      (assert-equal server-port (remote-port connection-request))
	      (assert-equal address (local-address connection-request))
	      (assert-equal client-port (local-port connection-request))
	      (assert-equal address (local-address accept))
	      (assert-equal server-port (local-port accept))
	      (assert-equal address (remote-address accept))
	      (assert-equal client-port (remote-port accept)))))))))
