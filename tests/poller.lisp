...
(define-test poller/interrupt
  ...
  (let ((finished-properly nil))
		 ...
		 (bordeaux-threads:make-thread #'(lambda ()
						   (with-poller (poller (make-poller))
						     (with-socket (s (make-ipv4-tcp-server +ipv4-loopback+ port))
						       (monitor-socket poller s 'connection-available-p)
						       (let ((rv (wait-for-events poller 60)))       
							 (setf finished-properly (list :poller rv))))))))))
	...
	(destructuring-bind (&optional keyword rv) finished-properly
	  (assert-eql :poller keyword)
	  (assert-true (null rv)))))))
